    ...
    <div class="row">
        @foreach($wholesaleProducts as $wholesaleProduct)
            <div class="col-lg-3 col-md-4 col-sm-6">
                <div class="card">
                    <img  src="{{ asset("images/".optional($wholesaleProduct->images->first())->product_image) }}" style="height: 160px"class="card-img-top w-100" alt="...">
                    <div class="card-body">
                        <p class="subSubExtraID"></p>
                        <p class="card-text text-center mb-4">{{ $wholesaleProduct->name }}</p>
                        <p class="text-center">{{ $wholesaleProduct->title }}</p>
                        <p class="text-center">{{ $wholesaleProduct->business_price }}</p>
                        <button type="button" id="singleDetailsId" onclick="getSubSubDetails({{ $wholesaleProduct->id }})" class="btn btn-success btn-block custom-modal-btn-margin">details > </button>
                        @if($wholesaleProduct->quantity > 0)
                            <a href="{{ url('add/to/cart') }}/{{ $wholesaleProduct->id }}"  type="button" id="singleDetailsCartId" onclick="addToCart({{ $wholesaleProduct->id }})"  class="btn btn-danger btn-lg btn-block add-cart" >Add to cart</a>
                        @else
